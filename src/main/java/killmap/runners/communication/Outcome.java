 ...
 * 
 * Records: the general kind of outcome (pass/fail/timeout/crash) the hashes of what was printed to
 * stdout/stderr how long the test took to run the stack trace of the failure (or "") the set of
 * covered mutants.
 */
public class Outcome {
  ...
  public static String normalizeStackTrace(String stackTrace) {
    // Given a multi-line stack trace, crushes it down to one line and removes potentially-variable
    // formatting. Ideally, any two stack traces which represent "the same error" should be
    // normalised to identical strings.
    return stackTrace.replaceAll(" *\r?\n[ \t]*", " ") // kill newlines and surrounding space
        .replaceAll("^[ \t\r\n]*|[ \t\r\n]*$", ""); // strip whitespace
  }

  public Outcome(Result result, String digest_) {
    // Constructs an Outcome from a JUnit result. Only PASS/FAIL outcomes can be made this way: if a
    // test times out or crashes the JVM, there won't be a JUnit result to base the Outcome on. In
    // those cases, use createTimeout or createCrash.
    digest = digest_;
  ...
  public boolean equals(Object otherObj) {
    ...
    return type == other.type && digest.equals(other.digest) && runTime.equals(other.runTime)
        && stackTrace.equals(other.stackTrace) && coveredMutants.equals(other.coveredMutants);
  }

  @Override
  public int hashCode() {
    return type.hashCode() + digest.hashCode() + runTime.hashCode() + stackTrace.hashCode()
        + coveredMutants.hashCode();
  }
  ...
  public static Outcome fromString(String s) throws IllegalArgumentException {
    ...
    // forall o:Outcome, Outcome.fromString(o.toString()).equals(o)
